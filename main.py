...
import scraper.agent, scraper.util, scraper.scrap, scraper.connector
...
async def proxy_server():
    # while True:
    proxy = scraper.util.get()
    proxy_host = proxy[0]
    proxy_port = proxy[1]
    # valid_proxies = []
        # if await scraper.connector.check_proxy(':'.join(proxy), 1, site, timeout, True, verbose, output, valid_proxies):
...
async def async_scrape():
    await scraper.scrap.scrape(method, file, verbose)
    await scraper.connector.check(file, timeout, method, site, verbose, True, output)
